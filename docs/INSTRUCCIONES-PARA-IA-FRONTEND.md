...
**Origen:** https://github.com/JosuePuentes/Zas-backend/blob/main/docs/INSTRUCCIONES-PARA-IA-FRONTEND.md
...
| **Headers con auth** | Añadir `Authorization: Bearer ` (token devuelto por login) |
...
- Guardar `access_token` y usarlo en **todas** las peticiones del panel admin: `Authorization: Bearer `.
...
Mostrar en el menú **solo** las secciones cuyo valor esté en el array `modulos` de la respuesta del login admin (o todas si `rol === "master"` o `modulos` incluye `"master"`):
